...
use crate::{
    ...
    infrastructure::database::{leads::LeadRepository, audit::AuditRepository, notifications::NotificationRepository},
    models::{lead::Lead, common::{PaginatedResponse, PaginationParams}},
};
...
pub async fn list_leads(
    ...
    let leads = repo.list(tenant_id, limit, offset, q)
        .await
...
pub async fn get_lead(
    ...
    let lead = repo.get_by_id(id, tenant_id)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
...
pub async fn update_lead(
    ...
    let old_lead = repo.get_by_id(id, tenant_id).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?.ok_or(StatusCode::NOT_FOUND)?;

    let lead = repo.update(
        id,
        tenant_id,
